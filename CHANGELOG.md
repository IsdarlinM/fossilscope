...
## 0.5.16 - 2026-08-10
- Fixed the real Windows Security Workspace failure where `/api/v1/workbench/catalog` returned HTTP 500 because modern Typer `TyperArgument` objects are not `click.Argument` subclasses. The shared SRIC 0.5.16 catalog now uses semantic `param_type_name` first and preserves positional arguments even though Typer 0.26 also exposes an `opts` attribute on them.
- Added a bounded FossilScope compatibility bridge for already-installed SRIC 0.5.14/0.5.15 runtimes so product-first repair cannot leave real `TyperArgument` fields misclassified as options while the shared core is being advanced.
- Added exhaustive CLI/Web parameter validation in addition to the existing 45-command functional smoke: every raw parameter is checked for concrete class, argument/option semantics, parser conversion, exact Web control mapping, unknown-option handling, missing-required-input handling and exact CLI/Web command/parameter parity.
- Added a fixed-Web-transport gate that submits every executable public operation through the real `WebConsoleManager` approval/classification path with execution threads replaced by deterministic no-side-effect test doubles; context-only operations must remain non-executable.
- Added product-level callback exception guardrails after the complete Typer tree is assembled. Expected file/value/schema failures become controlled exit-code 2 responses; unexpected callback failures are redacted and converted to controlled exit-code 1 responses instead of escaping to terminal or Web execution.
- Added explicit validation handling for plugin inspection, `collect-url`, time-travel, mobile archaeology, lifecycle assessment and reobservation plan/retry schemas.
- Adopted SRIC Core 0.5.16 shared Web guardrails: malformed catalog/coverage/submission/history/status/cancellation runtime failures become bounded redacted HTTP 503 responses, unexpected SSE runtime failures become terminal failed events, and the browser exposes catalog health plus a functional `Reload interface` recovery control.
