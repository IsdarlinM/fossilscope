...
The Windows installer smoke deliberately removes `annotated-types` from an installed runtime, proves that the broken runtime cannot start, executes `scripts\repair-windows.cmd`, and then requires `version`, `doctor --json`, `capabilities`, workspace preservation and cleanup of the temporary rollback runtime.
