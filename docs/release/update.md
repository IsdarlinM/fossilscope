...
Configure `FOSSILSCOPE_RELEASE_MANIFEST_URL` and `FOSSILSCOPE_RELEASE_PUBLIC_KEY`, or pass `--manifest` and `--public-key` explicitly.
...
`fossilscope update --check` verifies signed release metadata and reports availability without installing. `fossilscope update` installs a newer trusted release selected by the manifest. `fossilscope update --force` explicitly reinstalls the selected signed release even when that exact version is already installed; the updater invokes pip with `--force-reinstall` after signature and SHA-256 verification.
