...
if [ -n "${SRIC_CORE_SOURCE:-}" ]; then
  ...
  "$VENV/bin/python" -m pip install --upgrade --force-reinstall -c "$CONSTRAINTS" "$SRIC_CORE_SOURCE" || { echo "Failed to repair SRIC Core from SRIC_CORE_SOURCE." >&2; exit 3; }
else
...
"$VENV/bin/python" -c 'import importlib.metadata as m; import sric.web_console, sric.web_workbench; v=tuple(int(x) for x in m.version("sric-core").split(".")[:3]); raise SystemExit(0 if (0,5,7) <= v < (0,6,0) else 1)' || { echo "SRIC Core runtime integrity check failed; required >=0.5.7,<0.6 with Web Console and Workbench modules." >&2; exit 3; }
