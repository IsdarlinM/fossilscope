...
# SRIC is a normal package dependency. A source override is accepted only when explicitly requested for development/release validation.
...
"$VENV/bin/python" -m pip install --upgrade -c "$CONSTRAINTS" "$REPO_ROOT" || { echo "Installation failed. FossilScope requires sric-core>=0.5,<0.6, resolved automatically unless SRIC_CORE_SOURCE is set." >&2; exit 3; }
