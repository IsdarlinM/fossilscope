...
"%VENV%\Scripts\python.exe" -c "import annotated_types, pydantic, fossilscope; import importlib.metadata as m; assert m.version('fossilscope') == '0.5.16'; import sric.web_console, sric.web_workbench, sric.web_security_workspace, sric.web_catalog, sric.web_runtime, sric.web_theme, sric.web_guardrails; from sric.web_catalog import build_json_safe_command_catalog, install_json_safe_catalog; install_json_safe_catalog(); from sric.web_workbench import build_feature_catalog, feature_contract; v=tuple(int(x) for x in m.version('sric-core').split('.')[:3]); commands=build_json_safe_command_catalog('fossilscope.cli_all'); features=build_feature_catalog('fossilscope.cli_all'); contract=feature_contract('fossilscope.cli_all'); raise SystemExit(0 if (0,5,16)<=v<(0,6,0) and commands and len(commands)==len(features) and contract.get('complete') is True else 1)" || (echo FossilScope/SRIC Web runtime integrity check failed. Required fossilscope==0.5.16, sric-core ^>=0.5.16,^<0.6, and complete CLI/Web catalog parity.& goto :restore_runtime)
