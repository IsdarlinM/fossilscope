...
"%VENV%\Scripts\python.exe" -c "import annotated_types, pydantic, fossilscope; import importlib.metadata as m; assert m.version('fossilscope') == '0.5.14'; import sric.web_console, sric.web_workbench, sric.web_security_workspace, sric.web_catalog, sric.web_runtime; v=tuple(int(x) for x in m.version('sric-core').split('.')[:3]); raise SystemExit(0 if (0,5,14)<=v<(0,6,0) else 1)" || (echo FossilScope/SRIC runtime integrity check failed. Required fossilscope==0.5.14 and sric-core ^>=0.5.14,^<0.6.& goto :restore_runtime)
