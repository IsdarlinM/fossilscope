...
def test_help_variants_use_complete_entrypoint() -> None:
    for args in (["--help"], ["-h"], ["help"]):
        ...
        assert "fossils" in result.output
        assert "evolution-diff" in result.output
        assert "stale-references" in result.output
        assert "reobserve-plan" in result.output
