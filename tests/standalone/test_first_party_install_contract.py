...
def test_installers_resolve_product_and_first_party_atomically() -> None:
    ...
    # Regression for ResolutionImpossible: never force-reinstall the product in a
    # separate resolver invocation that omits the explicit first-party SRIC source.
...
def test_cross_platform_path_and_python_contract() -> None:
    windows = (ROOT / "scripts" / "install-windows.cmd").read_text(encoding="utf-8")
    linux = (ROOT / "scripts" / "install-linux.sh").read_text(encoding="utf-8")
