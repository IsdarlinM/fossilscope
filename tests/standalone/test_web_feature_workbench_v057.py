...
def test_every_public_cli_command_and_argument_is_represented_in_workbench() -> None:
    cli = {item["path"]: item for item in build_command_catalog("fossilscope.cli_all")}
    web = {item["path"]: item for item in build_feature_catalog("fossilscope.cli_all")}
...
def test_every_public_cli_command_help_exposes_options_and_required_arguments() -> None:
    catalog = build_command_catalog("fossilscope.cli_all")
...
def test_workbench_contract_and_dashboard_navigation(tmp_path) -> None:
    client = TestClient(create_app(tmp_path))
    dashboard = client.get("/")
    assert dashboard.status_code == 200
    assert "All Features" in dashboard.text
    assert "href='/workbench'" in dashboard.text
    assert "href='/console'" in dashboard.text
    ...
    assert {item["path"] for item in payload["features"]} == {
        item["path"] for item in build_command_catalog("fossilscope.cli_all")
    }


def test_native_temporal_features_remain_available(tmp_path) -> None:
    client = TestClient(create_app(tmp_path))
    for route in ("/api/timeline", "/api/candidates", "/api/lifecycle", "/api/graph", "/api/clusters", "/api/jobs", "/api/notebook"):
